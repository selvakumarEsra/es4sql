...
import org.elasticsearch.es4sql.domain.*;
import org.elasticsearch.es4sql.domain.hints.Hint;
import org.elasticsearch.es4sql.domain.hints.HintFactory;
        ...
        if (expr instanceof SQLBinaryOpExpr) {
            ...
            if(soExpr.getRight() instanceof SQLMethodInvokeExpr){
                ...
                if(Condition.OPEAR.methodNameToOpear.containsKey(methodName)){
                    ...
                    Condition condition = new Condition(Where.CONN.valueOf(opear) ,soExpr.getLeft().toString(), Condition.OPEAR.methodNameToOpear.get(methodName),methodParametersValue);
                    where.addWhere(condition);
                    methodAsOpear = true;
                }
            }
            if(!methodAsOpear){
                Condition condition = new Condition(Where.CONN.valueOf(opear), soExpr.getLeft().toString(), soExpr.getOperator().name, parseValue(soExpr.getRight()));
                where.addWhere(condition);
            }
        } else if (expr instanceof SQLInListExpr) {
            SQLInListExpr siExpr = (SQLInListExpr) expr;
            Condition condition = new Condition(Where.CONN.valueOf(opear), siExpr.getExpr().toString(), siExpr.isNot() ? "NOT IN" : "IN", parseValue(siExpr.getTargetList()));
            where.addWhere(condition);
        } else if (expr instanceof SQLBetweenExpr) {
            SQLBetweenExpr between = ((SQLBetweenExpr) expr);
            Condition condition = new Condition(Where.CONN.valueOf(opear), between.getTestExpr().toString(), between.isNot() ? "NOT BETWEEN" : "BETWEEN", new Object[]{parseValue(between.beginExpr),
            ...
            Condition condition = new Condition(Where.CONN.valueOf(opear), sqlIn.getExpr().toString(), sqlIn.isNot() ? "NOT IN" : "IN",subQueryExpression);
            where.addWhere(condition);
        } else {
